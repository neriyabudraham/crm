...
exports.getClients = async (req, res) => {
    ...
    let result;
    if (entity_type) {
        result = await db.query('SELECT * FROM clients WHERE entity_type = $1 ORDER BY created_at DESC', [entity_type]);
    } else {
        result = await db.query('SELECT * FROM clients ORDER BY created_at DESC');
    }
