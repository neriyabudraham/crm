...
router.get('/', async (req, res) => {
    try {
        const result = await db.query('SELECT * FROM questionnaires ORDER BY created_at DESC');
...
router.post('/', async (req, res) => {
    ...
    try {
        const result = await db.query(
            'INSERT INTO questionnaires (name, entity_type, fields, field_mapping) VALUES ($1, $2, $3, $4) RETURNING *',
            [name, entity_type || 'bride', JSON.stringify(fields || []), JSON.stringify(field_mapping || {})]
        );
...
router.patch('/:id', async (req, res) => {
    ...
    try {
        const result = await db.query(
            'UPDATE questionnaires SET name = COALESCE($1, name), fields = COALESCE($2, fields), field_mapping = COALESCE($3, field_mapping), entity_type = COALESCE($4, entity_type) WHERE id = $5 RETURNING *',
            [name, fields ? JSON.stringify(fields) : null, field_mapping ? JSON.stringify(field_mapping) : null, entity_type !== undefined ? entity_type : null, req.params.id]
        );
        res.json(result.rows[0]);
    } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.delete('/:id', async (req, res) => {
    try {
        await db.query('DELETE FROM questionnaires WHERE id = $1', [req.params.id]);
        res.json({ success: true });
    } catch (err) { res.status(500).json({ error: err.message }); }
});

// יצירת session שאלון עם טוקן
router.post('/sessions', async (req, res) => {
    const { questionnaire_id, client_id } = req.body;
    try {
        // בדיקה אם כבר קיים session לאותו שאלון ולקוח
        ...
        const result = await db.query(
            'INSERT INTO questionnaire_sessions (token, questionnaire_id, client_id) VALUES ($1, $2, $3) RETURNING *',
            [token, questionnaire_id, client_id]
        );
    ...
    try {
        const result = await db.query(
            `SELECT qs.*, q.name as questionnaire_name, q.fields as questionnaire_fields
             FROM questionnaire_sessions qs
             JOIN questionnaires q ON qs.questionnaire_id = q.id
             WHERE qs.client_id = $1 ORDER BY qs.created_at DESC`,
            [req.params.clientId]
        );
        res.json(result.rows);
    } catch (err) { res.status(500).json({ error: err.message }); }
